...
public class Book {
    ...
    public static List<Book> createListOfBooks(Integer booksQuantity) {
        ...
        for (int i = 0; i < booksQuantity; i++) {
            String randomNameOfBook = RandomStringUtils.randomAlphabetic(3, 10).toUpperCase();
            Integer randomPriceOfBook = new Random().nextInt(100);
            Book book = new Book(randomNameOfBook, randomPriceOfBook);
    ...
    public static List<String> createListOfNamesAllBooks(List<Book> listOfBooks) {
        return listOfBooks.stream()
                .map(Book::getName)
                .collect(Collectors.toList());
    }
